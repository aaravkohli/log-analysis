...
const LogSearchPanel = ({ logEntries }: LogSearchPanelProps) => {
  ...
  const [filteredLogs, setFilteredLogs] = useState<LogEntry[]>(logEntries);
  ...
  useEffect(() => {
    ...
    // Get unique values from data manager
    ...
    setUniqueIPs(Array.from(ips).sort());
  }, [logEntries]);
        ...
        <Card className="bg-blue-900/20 border-blue-500/30">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 text-blue-300 text-sm">
              ...
              <span>Searching through {dataSource.type} data • {logEntries.length.toLocaleString()} total entries</span>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white"
              >
                <option value="">All IPs ({uniqueIPs.length})</option>
                {uniqueIPs.slice(0, 20).map(ip => (
